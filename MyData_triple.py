...
class MyDataset(Dataset):  # 创建自己的类：MyDataset,这个类是继承的torch.utils.data.Dataset
    ...
    def __getitem__(self, index):
        ...
        falseimg = cv2.imread(self.falseroot + fn)
        tureimg = cv2.imread(self.tureroot + fn)# 按照path读入图片from PIL import Image # 按照路径读取图片
        falseimg = cv2.resize(falseimg,(224,224))
        tureimg = cv2.resize(tureimg, (224, 224))
        ...
        if self.transform is not None:
            ...
            tureimg = self.transform(tureimg)# 是否进行transform
        return rawimg, tureimg, falseimg  # return很关键，return回哪些内容，那么我们在训练时循环读取每个batch时，就能获得哪些内容
...
class TestDataset(Dataset):  # 创建自己的类：MyDataset,这个类是继承的torch.utils.data.Dataset
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            ...
            falseimg = self.transform(falseimg)
            tureimg = self.transform(tureimg)  # 是否进行transform
        return rawimg, tureimg, falseimg,label,fn  # return很关键，return回哪些内容，那么我们在训练时循环读取每个batch时，就能获得哪些内容
